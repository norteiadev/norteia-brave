...
**Status (parcial):** `website` CORRIGIDO — `_build_push_payload` agora lê
`canonical.get("website")` com fallback pra `contacts` (+ mesmo tratamento pra `instagram`);
teste `test_website_top_level_reaches_push_payload` em tests/unit/test_distrito_passthrough.py.
`phone`/`whatsapp` DEIXADO como estava (só `contacts.phone_e164`) — pendente decisão de produto:
telefone público (Places) → coluna `whatsapp` da norteia-api vs. campo phone dedicado.
