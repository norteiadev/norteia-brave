    ...
    async def produce(self, uf: str) -> None:
        ...
        for query in search_queries:
            ...
            for place in places_results:
                ...
                # Resolve parent destino destino-first: materialize (or fetch) the
                # authoritative IBGE destino on demand from the now-populated
                # municipio_ibge/municipio_nome (mirrors the TA lane). No quarantine —
                # the parent always exists after ensure_destino. Returns the parent's
                # Rio id + source_ref, and a Mar id only when the destino reached Mar.
                parent_rio_id, parent_source_ref, parent_mar_id = ensure_destino(
                    self._session,
                    self._config,
                    ibge_code=municipio_ibge,
                    nome=municipio_nome,
                    uf=uf,
                )
                ...
                payload: dict[str, Any] = {
                    ...
                    # Parent destino linkage (destino-first, TA-consistent): carry the
                    # Rio id + source_ref always, and the Mar id only when the destino
                    # already reached Mar (routing.py copies all three to normalized).
                ...
                if parent_mar_id is not None:
                    payload["parent_mar_id"] = str(parent_mar_id)
