...
def client(monkeypatch):
    ...
    # Mock the orchestrator dispatch so start never touches a broker.
...
def test_start_transitions_to_running(client):
    r = client.post("/api/v1/engine/start", headers=STEWARD_HEADERS)
...
def test_start_twice_returns_409(client):
    assert client.post("/api/v1/engine/start", headers=STEWARD_HEADERS).status_code == 202
    assert client.post("/api/v1/engine/start", headers=STEWARD_HEADERS).status_code == 409


@pytest.mark.integration
def test_stop_requests_graceful_stop(client):
    client.post("/api/v1/engine/start", headers=STEWARD_HEADERS)
...
def test_start_accepts_custom_ufs_and_lane(client):
    r = client.post(
        ...
        json={"ufs": ["BA", "RJ", "SP"], "lane": "destinos"},
    )
    assert r.status_code == 202
    body = r.json()
