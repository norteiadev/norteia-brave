...
def test_beat_entries_are_registered_without_a_queue_option() -> None:
    ...
    assert "collect-description-batches-15min" in entries
    assert "options" not in entries["collect-description-batches-15min"]
    # Descriptions are only generated by the Painel's describe action — never on a timer.
    assert not any(e["task"] == "brave.submit_description_batch" for e in entries.values())
